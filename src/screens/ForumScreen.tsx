...
const CATEGORIES: { value: string; label: string }[] = [
  ...
  { value: 'land_discussion', label: 'Public Land' },
  { value: 'strategy', label: 'Strategy' },
  { value: 'regulations', label: 'Regulations' },
  { value: 'gear', label: 'Gear' },
  { value: 'season_report', label: 'Season Report' },
];
...
export default function ForumScreen() {
  ...
  const threadTitleRef = useModalFocus(showNewThread);
  ...
  const renderThread = ({ item }: { item: ForumThread }) => {
    ...
    return (
      <TouchableOpacity style={styles.threadCard} activeOpacity={0.7}>
        ...
        {item.land_name && (
          <Text style={styles.landTag}>Land: {item.land_name}</Text>
        )}
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {activeTab === 'discussions' && (
        <FlatList
          horizontal
          data={[{ value: null as string | null, label: 'All' }, ...CATEGORIES]}
      ...
      {/* ── New Thread Modal ── */}
      <Modal visible={showNewThread} animationType="slide" transparent={false} onRequestClose={() => setShowNewThread(false)}>
