#!/usr/bin/env python3
"""
emit_ts_osm.py — Splices OSM-sourced polylines (from raw/osm_foot_trails.json)
into HIKING_TRACES, removes the matching IDs from HIKING_TRACE_GAPS, and
updates HIKING_TRACE_SUMMARY.

Idempotent via BEGIN/END sentinels — re-running replaces rather than stacks.
"""
...
OSM_PATH = SCRIPT_DIR / 'raw' / 'osm_foot_trails.json'
TRACES_TS = SCRIPT_DIR.parent.parent / 'src' / 'data' / 'marylandHikingTraces.ts'

BEGIN = '  // ── BEGIN OSM FOOT TRAILS (2026-04-19) ──'
END   = '  // ── END OSM FOOT TRAILS ──'
...
def fmt_trace(r: dict) -> str:
    ...
    # For out-and-back trails the polyline shows one direction; the
    # length-contract test (0.5-2.0 high, 0.25-4.0 ship) operates on
    # polyline-length / publishedMi, so publishedMi here is the
    # one-way (effective) length. The hike's round-trip mileage lives
    # on Trail.lengthMi for user display.
...
def main() -> None:
    ...
    results = data['results']  # dict trail_id -> result

    # Build the new trace block
    ...
    # HIKING_TRACES count: everything between the `= {` of HIKING_TRACES
    # and the `};` before AT_SEGMENT_TRACES.
    ...
    # byConfidence: count confidence tags inside HIKING_TRACES +
    # AT_SEGMENT_TRACES bodies.
    ...
    by_conf_json = '{' + ', '.join(f'"{k}": {v}' for k, v in sorted(conf.items())) + '} as const'
    new_summary = (
    ...
    print(f'  {len(results)} OSM traces spliced')
